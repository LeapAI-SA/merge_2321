...
class IrActionsReport(models.Model):
    ...
    def _render(self, report_ref, res_ids, data=None):
        ...
        if report.report_docx:
            mimetype, out, report_name, ext = self.render_any_docs(res_ids, report_ref, data=data)
            return out, ext
        else:
            return super(IrActionsReport, self)._render(report_sudo, res_ids, data)
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if report_sudo.report_docx:
            ...
            mimetype, out, report_name, ext = self.render_any_docs(res_ids, report_ref, data=data)
            return out, ext
        return super(IrActionsReport, self)._render_qweb_pdf(report_sudo, res_ids, data)
            ...
            if report_sudo.output_file == 'pdf':
                ...
                if report_sudo.merge_template_id:
                    if hasattr(doc, 'merge_report'):
                        if doc.merge_report:
                            pdf_content, ext = report_sudo.merge_template_id._render_qweb_pdf(doc.id)
